...
{
	// Load and configure shader
	this->TextShader = ResourceManager::LoadShader(GameParameters::resDirectory + "shaders/textVertex.txt", GameParameters::resDirectory + "shaders/textFragment.txt", "", "text");
	this->TextShader.Use();
...
void TextRenderer::RenderLabel(Label *label)
{
	...
	for (c = label->text.begin(); c != label->text.end(); c++)
	{
		...
		ypos = label->position.y + (this->Characters['H'].Bearing.y - ch.Bearing.y) * label->scale;
		...
		glDrawArrays(GL_TRIANGLES, 0, 6);
		// Now advance cursors for next glyph
		x += (ch.Advance >> 6) * label->scale; // Bitshift by 6 to get value in pixels (1/64th times 2^6 = 64)
	}
	glBindVertexArray(0);
	glBindTexture(GL_TEXTURE_2D, 0);
	this->TextShader.UnUse();
...
void TextRenderer::RenderButton(Button *button)
{
	...
	for (c = button->text.begin(); c != button->text.end(); c++)
	{
		...
		ypos = button->position.y + (this->Characters['H'].Bearing.y - ch.Bearing.y) * button->scale;
		...
		glDrawArrays(GL_TRIANGLES, 0, 6);
		// Now advance cursors for next glyph
		x += (ch.Advance >> 6) * button->scale; // Bitshift by 6 to get value in pixels (1/64th times 2^6 = 64)
	}
	glBindVertexArray(0);
	glBindTexture(GL_TEXTURE_2D, 0);
	this->TextShader.UnUse();

...
void Map::Load(std::string file)
{
         ...
         ++it)
    {
        ...
        std::string floorSpritePath =
            GameParameters::resDirectory + "textures/weapons/" +
            it->at("floorSprite").get<std::string>() + ".png";
        ResourceManager::LoadTexture(spritePath, GL_TRUE,
                                     it->at("sprite").get<std::string>());
        ResourceManager::LoadTexture(floorSpritePath, GL_TRUE,
                                     it->at("floorSprite").get<std::string>());
    }
         ...
         child = child->next_sibling())
    {
        ...
        const int xoffset =
            textureIndex % (ResourceManager::GetTexture("cs2dnorm").Width / 32);
        const int yoffset =
            textureIndex / (ResourceManager::GetTexture("cs2dnorm").Width / 32);
        const Sprite sprite = Sprite(ResourceManager::GetTexture("cs2dnorm"),
        ...
        if (itemId != 0)
        {
                 ...
                 it != weaponsJ.end(); ++it)
            {
                if (itemId == it->at("id").get<int>())
                {
                    ...
                    w = new Weapon(
                        pos, Sprite(ResourceManager::GetTexture(textureName)),
                        Sprite(ResourceManager::GetTexture(floorTextureName)),
                        name, type, maxAmmo, currentAmmo, curAmmoInMag,
                        maxAmmoInMag, dropable, ammoAndWeapon);
                    found = true;
                    break;
                }
            }
